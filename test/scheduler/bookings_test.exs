defmodule Scheduler.BookingsTest do
  ...
  describe "bookings" do
    ...
    test "create_booking/1 with valid data creates a booking" do
      valid_attrs = %{date: ~D[2025-08-23], time: ~T[14:00:00]}

      assert {:ok, %Booking{} = booking} = Bookings.create_booking(valid_attrs)
      assert booking.date == ~D[2025-08-23]
      assert booking.time == ~T[14:00:00]
    end
    ...
    test "update_booking/2 with valid data updates the booking" do
      ...
      assert booking.date == ~D[2025-08-24]
      assert booking.time == ~T[15:01:01]
    end
